...
export default defineConfig({
  testDir: './src',
  /* Run src in files in parallel */
  fullyParallel: true,
  /* Fail the build on CI if you accidentally left test.only in the source code. */
  forbidOnly: !!process.env.CI,
  /* Retry on CI only */
  retries: process.env.CI ? 2 : 0,
  /* Opt out of parallel src on CI. */
  workers: process.env.CI ? 1 : undefined,
  /* Reporter to use. See https://playwright.dev/docs/test-reporters */
  /* html は 1 件ごとの詳細を見るため、json は scripts/test-report.py がまとめるために使う */
  ...
  use: {
    ...
    /* 失敗した瞬間の画面を残す。レポートから何が出ていたかを追えるようにするため */
  ...
  /* Run your local dev server before starting the src */
  // webServer: {
